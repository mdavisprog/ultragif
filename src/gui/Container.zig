...
pub fn draw(self: *Self) void {
    ...
    // manually for now. However, with the debug view open, mouse wheel scrolling will be enabled
    // for scrolling the debug view panel.
    if (clay.isDebugModeEnabled()) {
        clay.updateScrollContainers(false, .init(mouse_wheel.x, mouse_wheel.y), self._delta_time);
    }
    clay.beginLayout();
